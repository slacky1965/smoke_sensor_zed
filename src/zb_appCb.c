...
void zb_bdbInitCb(uint8_t status, uint8_t joinedNetwork) {
    ...
    if (status == BDB_INIT_STATUS_SUCCESS) {
        ...
        if (joinedNetwork) {
            ...
            g_appCtx.timerPollRateEvt = TL_ZB_TIMER_SCHEDULE(poll_rateAppCb, NULL, TIMEOUT_30SEC);

            if (g_appCtx.timerStopReportEvt) {
                TL_ZB_TIMER_CANCEL(&g_appCtx.timerStopReportEvt);
            }
            g_appCtx.timerStopReportEvt = TL_ZB_TIMER_SCHEDULE(stopReportCb, NULL, TIMEOUT_15SEC);
    ...
    switch (status) {
        case BDB_COMMISSION_STA_SUCCESS:
            ...
            if (g_appCtx.timerNoJoinedEvt) {
                TL_ZB_TIMER_CANCEL(&g_appCtx.timerNoJoinedEvt);
            }
